...
"""
        ...
        self.assertEqual(
            noop_func.end_line, 7
        )  # End line is the same for single-line block
...
"""
        ...
        self.assertEqual(
            get_method.end_line, 11
        )  # Brace matching includes the final line
...
"""
        ...
        self.assertEqual(
            len(elements), 3
        )  # calculate func, Point class, display method
        ...
        constructor = self.helper.find_element(
            elements, ElementType.METHOD, "constructor"
        )
        self.assertIsNotNone(constructor)
        self.assertEqual(constructor.parent, point_class)

        display_method = (
            point_class.children[1] if len(point_class.children) > 1 else None
        )
...
"""
        ...
        impl_el = self.helper.find_element(
            elements, ElementType.IMPL, "Vec2"
...
"""
        ...
        process_func = self.helper.find_element(
            elements, ElementType.FUNCTION, "process"
        )
    ...
    def test_syntax_validity_check(self):
        ...
        invalid_code_brace = (
            "class T { void M() { if(true) {} } "  # Missing closing brace
        )
        invalid_code_paren = (
            "class T { void M() { if(true {} ) }"  # Missing closing paren
        )
        invalid_code_string = (
            'class T { void M() { char *s = "abc; } }'  # Unterminated string
        )
...
"""
        ...
        self.assertIsNotNone(
            self.helper.find_element(elements, ElementType.STRUCT, "Point")
        )
        self.assertIsNotNone(
            self.helper.find_element(elements, ElementType.STRUCT, "Rect")
        )
        self.assertIsNotNone(
            self.helper.find_element(elements, ElementType.FUNCTION, "draw")
        )
