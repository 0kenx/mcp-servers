...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate expected JSON for token parser tests"
    )
    parser.add_argument("source_file", help="Path to the source file")
    parser.add_argument(
        "--language", "-l", required=True, help="Language of the source file"
    )
    parser.add_argument(
        "--output",
        "-o",
        help="Output JSON file path (defaults to source_file.expected.json)",
    )
