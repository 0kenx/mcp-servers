...
class TestCTokenParser(unittest.TestCase):
    """Test class for C token parser."""

    def setUp(self):
        """Set up the test environment."""
        self.parser = ParserFactory.create_parser("c")
        self.test_data_dir = os.path.join("tests", "test_data", "c")
    ...
    def _serialize_elements(self, elements: List[CodeElement]) -> List[Dict[str, Any]]:
        ...
        for element in elements:
            ...
            if hasattr(element, "is_extern") and element.is_extern:
                serialized["is_extern"] = element.is_extern
