...
class TSReactTokenizer(TypeScriptTokenizer):
    ...
    def setup_jsx_rules(self):
        ...
        self.add_rule(TokenRule(r"/>", TokenType.JSX_TAG))

        # JSX tag end: >
        self.add_rule(TokenRule(r">(?![a-zA-Z0-9])", TokenType.JSX_TAG))

        # JSX expressions: {expression}
        self.add_rule(
            TokenRule(r"\{(?:[^{}]|\{[^{}]*\})*\}", TokenType.JSX_EXPRESSION, re.DOTALL)
        )

        # JSX text content
        self.add_rule(TokenRule(r"(?<=>)[^<{]+(?=<|\{|$)", TokenType.JSX_TEXT))
