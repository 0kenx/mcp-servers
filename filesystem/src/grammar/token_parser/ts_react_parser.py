...
class TSReactParser(TypeScriptParser):
    ...
    def _parse_jsx_element(
        ...
        self.symbol_table.add_symbol(
            ...
            metadata={
                "attributes": [attr["name"] for attr in attributes if "name" in attr]
            },
        )
        ...
        if not is_self_closing:
            while index < len(tokens):
                ...
                if tokens[index].token_type == TokenType.JSX_TAG and re.match(
                    r"</\s*" + re.escape(tag_name) + r"\s*>", tokens[index].value
                ):
        ...
        return {"node": jsx_element_node, "next_index": index}
    ...
    def _extract_jsx_attributes(self, tag_content: str) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(attr_pattern, content):
            ...
            attributes.append({"type": "JSXAttribute", "name": name, "value": value})
        ...
        for match in re.finditer(spread_pattern, content):
            attributes.append(
                {"type": "JSXSpreadAttribute", "expression": match.group(2).strip()}
            )
