            ...
            if i < len(tokens):
                if tokens[i].token_type == TokenType.KEYWORD:
                    ...
                    if tokens[i].value == "interface":
                        ...
                        # Parse type alias
                        type_alias, next_index = self._parse_type_alias_direct(
                            tokens, i
                        )
                        if type_alias:
            ...
            while (
                ...
                if tokens[index].token_type == TokenType.IDENTIFIER:
                    ...
                    while (
                        ...
                        and tokens[index].token_type != TokenType.OPEN_BRACE
                    ):
                        index += 1
    ...
    def _parse_type_alias_direct(
        ...
        while index < len(tokens):
            if tokens[index].token_type == TokenType.OPEN_BRACE:
                ...
                tokens[index].token_type == TokenType.OPERATOR
                and tokens[index].value == "<"
            ):
                angle_depth += 1
            elif (
                ...
                tokens[index].token_type == TokenType.SEMICOLON
                and brace_depth == 0
                and angle_depth == 0
                and paren_depth == 0
            ):
