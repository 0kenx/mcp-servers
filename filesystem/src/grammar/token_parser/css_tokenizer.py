...
class CSSTokenizer(Tokenizer):
    ...
    def setup_css_rules(self):
        ...
        self.add_rule(TokenRule(r"@[a-zA-Z-]+", TokenType.AT_RULE))

        # Property names (inside rules)
        self.add_rule(TokenRule(r"[a-zA-Z-]+(?=\s*:)", TokenType.PROPERTY))
        ...
        # ID selectors
        self.add_rule(TokenRule(r"#[a-zA-Z][a-zA-Z0-9_-]*", TokenType.SELECTOR))
        ...
        # Element selectors
        self.add_rule(
            TokenRule(r"[a-zA-Z][a-zA-Z0-9_-]*(?=[\s{:+~>\[,]|$)", TokenType.SELECTOR)
        )

        # Pseudo-classes and pseudo-elements
        self.add_rule(
            TokenRule(r":{1,2}[a-zA-Z][a-zA-Z0-9_-]*(?:\([^)]*\))?", TokenType.SELECTOR)
        )

        # Attribute selectors
        self.add_rule(TokenRule(r"\[[^\]]+\]", TokenType.SELECTOR))

        # Combinators
        self.add_rule(TokenRule(r"[>+~]", TokenType.OPERATOR))

        # Important declaration
        self.add_rule(TokenRule(r"!important", TokenType.IDENTIFIER))
        ...
        self.add_rule(
            TokenRule(
                r"#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})", TokenType.NUMBER
            )
        )

        # Functions like rgb(), calc(), url(), etc.
        self.add_rule(TokenRule(r"[a-zA-Z-]+\([^)]*\)", TokenType.IDENTIFIER))
