...
class TokenParser(BaseParser):
    ...
    def _check_for_overlapping_elements(self) -> None:
        ...
        sorted_elements = sorted(
            self.elements, key=lambda e: (e.start_line, e.end_line)
        )
    ...
    def _validate_parent_child_relationships(self) -> None:
        ...
        for element in self.elements:
            ...
            if element.parent:
                if not (
                    element.parent.start_line <= element.start_line
                    and element.parent.end_line >= element.end_line
                ):
    ...
    def _find_most_likely_parent(self, element: CodeElement) -> Optional[CodeElement]:
        ...
        for potential_parent in self.elements:
            ...
            if (
                potential_parent.start_line <= element.start_line
                and potential_parent.end_line >= element.end_line
            ):
    ...
    def _get_token_context(
        self, tokens: List[Token], index: int, count: int
    ) -> List[Token]:
        """
