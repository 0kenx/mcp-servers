...
class HTMLTokenizer(Tokenizer):
    ...
    def setup_html_rules(self):
        ...
        self.add_rule(
            TokenRule(
                r"<!DOCTYPE(?:.|\n)*?>", TokenType.DOCTYPE, re.DOTALL | re.IGNORECASE
            )
        )
        ...
        # Closing tags
        self.add_rule(TokenRule(r"</[a-zA-Z][a-zA-Z0-9_:.-]*\s*>", TokenType.CLOSE_TAG))
        ...
        # HTML entities
        self.add_rule(TokenRule(r"&[a-zA-Z0-9#]+;", TokenType.ENTITY))

        # Text content (anything that's not a tag, comment, etc.)
        self.add_rule(TokenRule(r"[^<&\s]+", TokenType.TEXT))

        # Whitespace
        self.add_rule(TokenRule(r"\s+", TokenType.WHITESPACE))
