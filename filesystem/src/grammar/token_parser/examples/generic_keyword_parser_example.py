...
def create_tokens_from_code(code: str) -> List[Token]:
    ...
    for word in words:
        if word == '\n':
            ...
            tokens.append(Token(TokenType.KEYWORD, word.lower(), position, line, column))
            column += len(word)
...
def parse_keyword_block(code: str) -> None:
    ...
    print("All Tokens:")
    for i, token in enumerate(tokens):
        print(f"  {i}: {token.token_type.name}: '{token.value}' at line {token.line}, column {token.column}")
    
    # Print the block tokens
    print("\nKeyword Block Contents:")
    for idx in block_indices:
        token = tokens[idx]
        print(f"  {idx}: {token.token_type.name}: '{token.value}' at line {token.line}, column {token.column}")
