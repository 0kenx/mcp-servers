...
def create_tokens_from_code(code: str) -> List[Token]:
    ...
    for line_num, line_content in enumerate(lines):
        ...
        if rest_of_line:
            ...
            tokens.append(
                Token(
                    TokenType.IDENTIFIER, rest_of_line, position, line_num + 1, column
                )
            )
...
def parse_indentation_block(code: str) -> None:
    ...
    for i, token in enumerate(tokens):
        indent_info = (
            f", indent={token.metadata.get('indent_size')}"
            if token.token_type == TokenType.WHITESPACE
            and "indent_size" in token.metadata
            else ""
        )
        print(
            f"  {i}: {token.token_type.name}: '{token.value}' at line {token.line}, column {token.column}{indent_info}"
        )
    ...
    for idx in block_indices:
        ...
        print(
            f"  {idx}: {token.token_type.name}: '{token.value}' at line {token.line}, column {token.column}{indent_info}"
        )
