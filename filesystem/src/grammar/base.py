...
class BaseParser:
    ...
    def find_function(self, code: str, name: str) -> Optional[CodeElement]:
        ...
        for element in elements:
            if element.element_type in (ElementType.FUNCTION, ElementType.METHOD) and element.name == name:
                return element
        return None
    
    def find_function_at_line(self, code: str, line_number: int) -> Optional[CodeElement]:
        """
        ...
        elements = self.parse(code)
        for element in elements:
            if element.element_type in (ElementType.FUNCTION, ElementType.METHOD) and element.contains_line(line_number):
                return element
        return None
    ...
    def get_functions(self, code: str) -> List[CodeElement]:
        ...
        """
        elements = self.parse(code)
        return [e for e in elements if e.element_type in (ElementType.FUNCTION, ElementType.METHOD)]
    ...
    @staticmethod
    def _strip_comments(code: str, single_line_comment: str, multi_line_start: Optional[str] = None, 
                        multi_line_end: Optional[str] = None) -> str:
        """
            ...
            if not in_multi_line:
                ...
                if multi_line_start and multi_line_start in line and multi_line_end:
                    ...
                    if len(parts) % 2 == 1:
                        ...
                        end_pos = line.find(multi_line_end, start_pos + len(multi_line_start))
                        
                        if end_pos != -1:
                            # Multi-line comment starts and ends on the same line
                            line = line[:start_pos] + " " * (end_pos + len(multi_line_end) - start_pos) + line[end_pos + len(multi_line_end):]
                        else:
                ...
                if multi_line_end and multi_line_end in line:
                    ...
                    line = " " * (end_pos + len(multi_line_end)) + line[end_pos + len(multi_line_end):]
                    in_multi_line = False
                else:
