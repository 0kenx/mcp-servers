...
This module provides a comprehensive parser for TypeScript code that can handle
...
class TypeScriptParser(JavaScriptParser):
    ...
    def _extract_class_methods(
        ...
        methods = super()._extract_class_methods(
            lines, start_idx, end_idx, parent_class, line_comments
        )
        ...
        while line_idx < end_idx:
            ...
            if ":" in line and ";" in line and "(" not in line:
                ...
                if property_match:
                    ...
                    is_private = (
                        "private" in line and "private" in line.split(property_name)[0]
                    )
                    is_protected = (
                        "protected" in line
                        and "protected" in line.split(property_name)[0]
                    )
                    is_readonly = (
                        "readonly" in line
                        and "readonly" in line.split(property_name)[0]
                    )
    ...
    def _process_decorators(self, elements):
        ...
        class_elements = {
            e.name: e
            for e in elements
            if e.element_type in [ElementType.CLASS, ElementType.INTERFACE]
        }
        ...
        for element in elements:
            if (
                element.metadata
                and "decorators" in element.metadata
                and element.metadata["decorators"]
            ):
    ...
    def parse(self, code: str) -> List[CodeElement]:
            ...
            if user_class:
                ...
                for element in elements:
                    if (
                        ...
                        and element.parent == user_class
                        and element.name == "getInfo"
                    ):
                        method_found = True
                        break
        ...
        while line_idx < line_count:
            ...
            if interface_match:
                ...
                extends_clause = (
                    interface_match.group(2) if interface_match.group(2) else None
                )
            ...
            if enum_match:
                ...
                element = CodeElement(
                    ...
                    metadata={
                        ...
                        and "const" in line.split("enum")[0],
                    },
                )
            ...
            if stack and stack[-1].element_type == ElementType.CLASS:
                ...
                if not property_match and (":" in line) and (";" in line):
                    ...
                    simplified_match = re.match(
                        r"\s*(?:private|protected|public|readonly)?\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*:\s*([^;]*);",
                        line,
                    )
                    if simplified_match:
                ...
                if element.element_type == ElementType.IMPORT:
                    ...
                    if imported_items:
                        ...
                        if imported_items.startswith("{"):
                            items = [
                                item.strip()
                                for item in imported_items.strip("{}").split(",")
                            ]
                            for item in items:
