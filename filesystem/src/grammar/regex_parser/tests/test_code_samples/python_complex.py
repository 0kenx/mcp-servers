...
from typing import (                                 #  -> DETECTION ERROR: import: typing (lines 27-27)
    Any, Callable, Dict, Generic, Iterable, List, Literal, Mapping,
    Optional, Protocol, Sequence, Set, Tuple, Type, TypeVar, Union,
    Annotated, cast, get_type_hints, overload, runtime_checkable
)
...
class ValidatedField:
    ...
    def __init__(self, field_type: Type, validator: Optional[Callable[[Any], bool]] = None):
...
class SortableContainer(DataContainer):
    ...
    def sort(self, key: Optional[Callable[[Any], Any]] = None, reverse: bool = False) -> 'SortableContainer':
            ...
            while attempt < max_attempts:
                try:
                    ...
                    if attempt < max_attempts:
                        logger.warning(f"Attempt {attempt} failed: {e}. Retrying in {current_delay:.2f} seconds...")
...
def complex_list_operations(data: List[Any]) -> Tuple[List[Any], List[Any], List[Any]]:
    ...
    transformed = [
        x * 2 if isinstance(x, int) else
        x.upper() if isinstance(x, str) else
        str(x)
        for x in data
    ]
...
def main():
    ...
    # Using processor
    processor = DataProcessor("test-processor", lambda x: x.upper() if isinstance(x, str) else x)
