...
def check_git_status() -> Tuple[bool, str]:
    ...
    if stdout:
        return False, "Git working directory is not clean. Please commit or stash your changes before continuing."
...
def get_github_repo() -> Tuple[bool, str]:
    ...
    stdout, stderr, rc = run_command(["git", "remote", "get-url", "origin"], check=False)
    ...
    try:
        ...
        for issue in all_issues:
            ...
            if "blocked" not in label_names:
                ...
                if pr_rc != 0:
                    return False, [], f"Error checking linked PRs for issue #{issue['number']}: {pr_stderr}"
                ...
                try:
                    ...
                    return False, [], f"Error parsing linked PR data for issue #{issue['number']}: {pr_stdout}"
...
def select_issue(issues: List[Dict[Any, Any]], issue_number: Optional[int] = None) -> Tuple[bool, Dict[Any, Any], str]:
    """
    ...
    if issue_number:
        ...
        return False, {}, f"Issue #{issue_number} not found or is not an eligible vibe issue."
...
def create_branch(issue_number: int, issue_title: str, source_branch: str) -> Tuple[bool, str]:
    """Create a new branch for the issue."""
    # Sanitize the issue title to create a valid branch name
    sanitized_title = re.sub(r'[^a-zA-Z0-9_-]', '-', issue_title.lower())
    sanitized_title = re.sub(r'-+', '-', sanitized_title)  # Replace multiple dashes with one
    ...
    check_stdout, check_stderr, check_rc = run_command(["git", "rev-parse", "--verify", f"origin/{source_branch}"], check=False)
    if check_rc != 0:
...
async def vibe_commit_fix(changelog: str) -> str:
    ...
    comment_cmd = ["gh", "issue", "comment", issue_number, "--repo", repo, "--body", f"## Changelog\n{changelog}"]
    ...
    status_stdout, status_stderr, status_rc = run_command(["git", "status", "--porcelain"], check=False)
    if status_rc != 0:
